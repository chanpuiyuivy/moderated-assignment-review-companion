...
def _mistakes(student: StudentScript, questions: list[Question]) -> list[str]:
    ...
    for qid, ans in student.answers.items():
        ...
        if ans.awarded < q.marks:
            ...
            lines.append(
                f"- Q{qid} ({q.question_type}, lost {lost:g}/{q.marks:g}): "
...
def _improvement(student: StudentScript, questions: list[Question]) -> str:
    ...
    for qid, ans in student.answers.items():
        ...
        if q and ans.awarded < q.marks:
            weak[q.question_type] += 1
    if not weak:
        return "This student met the marking scheme on every question. Stretch them with harder items of the same type."
    top = ", ".join(f"{k} ({v})" for k, v in weak.most_common())
    advice = {
        "mcq": "review option-elimination and key vocabulary",
        "short": "practise concise, complete statements that match command words",
        "long": "plan answers with point–evidence–explain and check against the mark scheme",
        "calculation": "show every step, include units, and estimate whether the magnitude is sensible",
    }
    tips = "; ".join(advice.get(k, "targeted practice") for k, _ in weak.most_common())
    return f"Weakest question types: {top}. Focus next: {tips}."
...
def question_report(students: list[StudentScript], questions: list[Question]) -> str:
    ...
    for q in questions:
        chunks.append(f"## Question {q.id} ({q.question_type}, {q.marks:g} mark(s))")
        ...
        type_loss[q.question_type].append(1 - (avg / q.marks if q.marks else 0))
        ...
        chunks.append("")
    chunks.append("## Weak question types")
    ranked = sorted(
        ((k, round(sum(v) / len(v), 3)) for k, v in type_loss.items() if v),
        key=lambda x: -x[1],
    )
    if not ranked:
        chunks.append("Not enough data.")
    else:
        chunks.append("Higher values mean a larger share of marks lost on that type:")
        for qtype, loss in ranked:
            chunks.append(f"- {qtype}: mean mark-loss ratio {loss:.0%}")
    return "\n".join(chunks)
