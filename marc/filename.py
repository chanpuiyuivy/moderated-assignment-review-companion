...
NAME_RE = re.compile(
    r"(?i)(?:student\s*)?name\s*[:\-.]?\s*([A-Za-z][A-Za-z .'\-]{1,48}?)(?=\s{2,}|\s+class\b|\s+(?:class\s*)?(?:no\.?|number|#)\b|$)",
)
CLASS_RE = re.compile(
    r"(?i)class(?!\s*(?:no|number|#))\s*[:\-.]?\s*([0-9]{1,2}\s*[A-Za-z]|[A-Za-z]\.?\s*[0-9]{1,2})"
)
CLASSNO_RE = re.compile(
    r"(?i)(?:class\s*)?(?:no\.?|number|#)\s*[:\-.]?\s*([0-9]{1,3})\b"
)
