...
from marc.parse_scheme import finalize_questions, parse_scheme_text, questions_from_openai_dicts
...
def page_students(engine: str, openai_key: str, model: str) -> None:
    ...
    if st.button("Read scripts (OCR)", type="primary", disabled=not files):
        ...
        for i, f in enumerate(files):
            ...
            try:
                text, warn = transcribe_student_pdf(data, engine, key, model)
...
def page_scheme(engine: str, openai_key: str, model: str) -> None:
    ...
    uploaded = st.file_uploader("Marking scheme", type=["pdf", "docx"], key="scheme_file")
    pasted = st.text_area("Or paste the scheme text", height=180, key="scheme_paste")
    ...
    if st.button("Parse scheme", type="primary"):
        ...
        st.success(f"Recognised {len(st.session_state.questions)} question(s). Continue to review them.")

    if st.session_state.questions:
        st.dataframe(questions_to_frame(st.session_state.questions), use_container_width=True, hide_index=True)
    ...
    nav_row(
        back=0,
        next_step=2,
        next_label="Next: review scheme",
        next_disabled=not st.session_state.questions,
    )


def page_review_scheme() -> None:
    st.header("Review questions and suggested answers")
    st.write(
        "Correct any parsing mistakes. Fill-in-the-blank items may show “(Fill in the blank)” "
        "with only the answer. Name, class, and class number are omitted. Default mark is 1."
    )
    if not st.session_state.questions:
        st.info("Parse a marking scheme first.")
        nav_row(back=1, next_step=None, next_label="Next", next_disabled=True)
        return
    ...
    if left.button("Back", key="nav_back"):
        go(1)
    if right.button("Next: moderate answers", type="primary", key="nav_next"):
...
def page_moderate() -> None:
    ...
    if not students or not questions:
        ...
        nav_row(back=2, next_step=None, next_label="View final report", next_disabled=True)
        return
    ...
    with tab_q:
        st.caption("Each question is listed in order. Accept any alternative wording and set its marks.")
        collected: list[AcceptedVariant] = []
        for q in questions:
            st.markdown(f"### Question {q.id}")
            prompt = q.prompt.strip() or "(Fill in the blank)"
            st.write(prompt)
            st.write(f"**Official answer:** {q.suggested_answer or '(none)'} · **{q.marks:g}** mark(s)")
            clusters = cluster_answers(students, q.id)
            existing = [v for v in st.session_state.variants if v.question_id == q.id]
            alternatives = [c for c in clusters if c["label"] not in (q.suggested_answer, "")]
            if not alternatives:
                st.write("_No alternative answers recorded._")
            for i, cluster in enumerate(alternatives):
                cols = st.columns([4, 1, 1])
                with cols[0]:
                    st.write(f"{cluster['count']} student(s): {cluster['label'][:400]}")
                ...
                if accept and cluster["canonical"]:
                    collected.append(
                        AcceptedVariant(question_id=q.id, text=cluster["canonical"], marks=marks, enabled=True)
                    )
            extra = st.text_area("Add another accepted wording", key=f"extra_{q.id}")
            extra_marks = st.number_input(
                "Marks for extra wording",
                min_value=0.0,
                max_value=float(q.marks),
                value=float(q.marks),
                key=f"exm_{q.id}",
            ...
            if extra.strip():
                collected.append(
                    AcceptedVariant(question_id=q.id, text=extra.strip(), marks=extra_marks, enabled=True)
                )
            st.divider()

        if st.button("Apply accepted answers and re-mark", type="primary", key="apply_alts"):
        ...
        for idx, q in enumerate(questions):
            ...
            with st.expander(f"Q{q.id} — {ans.awarded:g}/{q.marks:g}  ({ans.match_reason})"):
                ...
                ans.awarded = st.number_input(
                    ...
                    step=0.5,
                    key=f"aw_{student.filename}_{idx}_{q.id}",
                )

        if st.button("Re-run automatic marking", key="remark_student"):
            apply_marking(students, questions, st.session_state.variants)
            st.rerun()
...
def page_reports() -> None:
    ...
    if not students or not questions:
        ...
        nav_row(back=3, next_step=None, next_label="Next", next_disabled=True)
        return
