...
def _load_local_package() -> str:
    """Load the app package from this repo, not a PyPI module with the same name."""
    for name in ("marcapp",):
        ...
        for key in list(sys.modules):
            ...
            loc = getattr(other, "__file__", None)
            if not loc:
                sys.modules.pop(key, None)
                continue
            try:
                Path(loc).resolve().relative_to(pkg_dir)
            except ValueError:
                sys.modules.pop(key, None)
    ...
    raise ModuleNotFoundError(
        "App package folder missing next to app.py (expected marcapp/)."
    )


_PKG = _load_local_package()
filename = importlib.import_module(f"{_PKG}.filename")
marking = importlib.import_module(f"{_PKG}.marking")
models = importlib.import_module(f"{_PKG}.models")
ocr = importlib.import_module(f"{_PKG}.ocr")
parse_scheme = importlib.import_module(f"{_PKG}.parse_scheme")
parse_student = importlib.import_module(f"{_PKG}.parse_student")
pdf_io = importlib.import_module(f"{_PKG}.pdf_io")
reports = importlib.import_module(f"{_PKG}.reports")

parse_student_filename = filename.parse_student_filename


def filename_has_class_info(name: str) -> bool:
    helper = getattr(filename, "filename_has_class_info", None)
    if helper:
        return helper(name)
...
def extract_identity_from_text(text: str) -> tuple[str, str, str]:
    helper = getattr(filename, "extract_identity_from_text", None)
    if helper:
        return helper(text)
    blob = text or ""
    name = ""
    cls = ""
    num = ""
    m = re.search(r"(?i)\b(?:student\s*)?name\s*[:\-.]?\s*([A-Za-z][A-Za-z .'\-]{1,48})", blob)
    if m:
...
apply_marking = marking.apply_marking
cluster_answers = marking.cluster_answers
is_accepted_answer = marking.is_accepted_answer
score_answer = marking.score_answer
similarity = marking.similarity
AcceptedVariant = models.AcceptedVariant
Question = models.Question
StudentScript = models.StudentScript
openai_extract_scheme = ocr.openai_extract_scheme
resolve_openai_key = ocr.resolve_openai_key
transcribe_scheme_pdf = ocr.transcribe_scheme_pdf
transcribe_student_pdf = ocr.transcribe_student_pdf
FILL_IN = parse_scheme.FILL_IN
finalize_questions = parse_scheme.finalize_questions
parse_scheme_text = parse_scheme.parse_scheme_text
questions_from_openai_dicts = parse_scheme.questions_from_openai_dicts
attach_answers = parse_student.attach_answers
extract_scheme_text = pdf_io.extract_scheme_text
class_stats = reports.class_stats
individual_report = reports.individual_report
max_total = reports.max_total
overall_report = reports.overall_report
question_report = reports.question_report
student_sort_key = reports.student_sort_key
...
def questions_to_frame(questions: list[Question]) -> pd.DataFrame:
    if not questions:
        return pd.DataFrame(columns=["id", "prompt", "suggested_answer", "marks"])
    return pd.DataFrame(
        [
            {
                "id": q.id,
...
def page_students(engine: str, openai_key: str, model: str) -> None:
    ...
    if st.session_state.processed and st.session_state.students:
...
def page_scheme(engine: str, openai_key: str, model: str) -> None:
    ...
    st.caption("Tick the questions you want to remove, then click **Delete selected**.")
    st.markdown(
        """
        <style>
        div[data-testid="stCheckbox"] { transform: scale(1.2); margin-top: 0.4rem; }
        </style>
        """,
        unsafe_allow_html=True,
    )
    epoch = st.session_state.scheme_editor_epoch
    hdr = st.columns([0.7, 0.85, 2.6, 2.6, 0.85])
    hdr[0].markdown("**Sel**")
    hdr[1].markdown("**ID**")
    hdr[2].markdown("**Question**")
    hdr[3].markdown("**Suggested answer**")
    hdr[4].markdown("**Marks**")

    built: list[Question] = []
    for i, q in enumerate(st.session_state.questions):
        cols = st.columns([0.7, 0.85, 2.6, 2.6, 0.85])
        cols[0].checkbox("Select", value=False, key=f"sch_sel_{epoch}_{i}")
        qid = cols[1].text_input("ID", value=q.id, key=f"sch_id_{epoch}_{i}", label_visibility="collapsed")
        prompt = cols[2].text_input("Question", value=q.prompt, key=f"sch_pr_{epoch}_{i}", label_visibility="collapsed")
        answer = cols[3].text_input(
            "Suggested answer", value=q.suggested_answer, key=f"sch_ans_{epoch}_{i}", label_visibility="collapsed"
        )
        marks = cols[4].number_input(
            "Marks",
            min_value=0.0,
            value=float(q.marks),
            step=0.5,
    ...
    selected = {i for i in range(len(built)) if st.session_state.get(f"sch_sel_{epoch}_{i}")}
    act1, act2, _ = st.columns([1.8, 1.3, 3])
    if act1.button("Delete selected", disabled=not selected, key="sch_del_multi"):
        st.session_state.questions = [q for i, q in enumerate(built) if i not in selected]
        st.session_state.scheme_editor_epoch += 1
        st.rerun()
    if act2.button("Add question", key="sch_add"):
        used = {q.id for q in built}
        nxt = 1
        while str(nxt) in used:
            nxt += 1
        built.append(Question(id=str(nxt), prompt=FILL_IN, suggested_answer="", marks=1.0))
    ...
    with tab_q:
        ...
        for q in questions:
            ...
            with st.container(border=True):
                st.markdown(f"**Question {q.id}** · {q.marks:g} mark(s)")
...
def _render_by_question(students: list[StudentScript], questions: list[Question]) -> None:
    ...
    for q in questions:
        with st.container(border=True):
            ...
            clusters = cluster_answers(students, q.id)
            long_answers = any(len(c["label"]) > 80 for c in clusters)
            if long_answers:
                for c in clusters:
