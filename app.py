...
def _pdf_links(scripts: list[StudentScript], key_prefix: str) -> None:
    ...
    if not with_pdf:
        st.caption("No source PDFs stored for these students.")
        return
    cols = st.columns(min(len(with_pdf), 4))
    for i, student in enumerate(with_pdf):
        cols[i % 4].download_button(
            label=student.student_id,
            data=student.pdf_bytes,
            file_name=student.filename,
            mime="application/pdf",
            key=f"{key_prefix}_{student.filename}_{i}",
        )
...
def page_moderate() -> None:
    ...
    with tab_q:
        st.caption("Questions are listed in order. Tick **Accept** to award marks for that wording. Statistics update immediately.")
        for q in questions:
            with st.container(border=True):
                ...
                if not alternatives:
                    st.caption("No alternative answers.")
                ...
                extra = st.text_input("Add another accepted wording", key=f"extra_{q.id}")
    ...
    with tab_s:
        ...
        if student.pdf_bytes:
            meta2.download_button(
                "Open this student’s PDF",
                data=student.pdf_bytes,
                file_name=student.filename,
                mime="application/pdf",
                key=f"stu_pdf_{student.filename}",
            )
        ...
        rows.append(
            {
                "ID": q.id,
                "Question": q.prompt,
                "Student answer": ans.raw_text if ans else "",
                "Expected": q.suggested_answer,
                "Mark": ans.awarded if ans else 0,
                "Out of": q.marks,
            }
        )
    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
    st.markdown("**Mistakes**")
    for line in mistakes_for(current, questions) or ["None recorded."]:
        st.write(line)
    st.markdown("**Room for improvement**")
    st.write(improvement_for(current, questions))
