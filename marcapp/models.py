...
class StudentScript:
    ...
    def student_id(self) -> str:
        return f"{self.class_name}-{self.class_number}" if self.class_number else (self.student_name or self.filename)
    ...
    def display_label(self) -> str:
        ...
        if self.class_name and self.class_name != "UNKNOWN":
            bits.append(f"{self.class_name}-{self.class_number}" if self.class_number else self.class_name)
        return " · ".join(bits) or self.filename
