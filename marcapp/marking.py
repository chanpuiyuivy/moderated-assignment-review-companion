...
def is_accepted_answer(text: str, question: Question, variants: list[AcceptedVariant]) -> bool:
    keys = [question.suggested_answer] + [
        v.text for v in variants if v.enabled and v.question_id == question.id and v.text.strip()
    ]
    return any(similarity(text, key) >= 0.82 for key in keys if key.strip())
